...
class CleanGarbage(Node):
    ...
    def action_server_callback(self, goal_handle):
       ...
       # =======================================================================
        ...
        while True:
            # 误识别早退：2秒内work_status仍为idle，视为误识别，直接退出
            if (time.time() - self.receive_signal_time > 2.0
                and self.work_status == 'idle'
                and self.map is not None):
                self.get_logger().info('No garbage detected within 2s, considered as false positive, exiting...')
                break
